...
func (generator *Generator) file(from jen.Code, packageName string) *jen.File {
...
func (generator *Generator) Generate(swagger *openapi3.Swagger) *Result {
	return &Result{
		ComponentsCode: generator.file(generator.components(swagger), generator.packageName(generator.config.ComponentsPackageName)),
		RouterCode: generator.file(jen.Null().
			Add(generator.wrappers(swagger)).
			Add(jen.Line()).
			Add(generator.requestResponseBuilders(swagger)), generator.config.PackageName),
	}
}
...
func (generator *Generator) requestParameterStruct(name string, contentType string, appendContentTypeToName bool, operation *openapi3.Operation) jen.Code {
	...
	if contentType != "" {
		...
		additionalParameters = append(additionalParameters, parameter{In: "Body", Code: jen.Id("Body").
			Qual(generator.config.ComponentsPackageName, bodyTypeName)})
	}
				...
				linq.From(resp.ContentTypeBodyNameMap).
					SelectT(func(kv linq.KeyValue) jen.Code {
						...
						result = append(result, jen.Func().Params(
							...
							jen.Id("body").Qual(generator.config.ComponentsPackageName, cast.ToString(kv.Value))).Params(
							jen.Op("*").Id(assemblerName)).Block(
...
func (generator *Generator) packageName(from string) string {
